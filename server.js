...
Sorular.find().sort({createdAt : -1}).then((sresult)=>{
Kullanicilar.find().then((kkresult)=>{
if(kullaniciId != null){
Kullanicilar.findById(req.cookies.id).then((kresult)=>{
res.render(__dirname+"/src/signed/index.ejs", {title : "Anasayfa", kullanici : kresult, soru : sresult})  
})
} else {
res.render(__dirname+"/src/pages/index.ejs", {title : "Anasayfa", soru : sresult})  
}
})
}).catch((err)=>{console.log(err)})
})
...
if(admin == "true"){
Sorular.find().sort({createdAt : -1}).then((sresult)=>{
Sorular.find().count().then((slresult)=>{
res.render(__dirname+'/src/admin/dashboard.ejs', { soru : sresult, kullanici : result, title : "Dashboard", sorusayi : slresult})
})
})
} else {
res.redirect("/")
...
Soruyorum.find({soruId : id}).sort({createdAt : -1}).then((yresult)=>{
Kullanicilar.find().then((kkresult)=>{
if(kullaniciId != null){
Kullanicilar.findById(req.cookies.id).then((kresult)=>{
var goruntulenme = Number(result.goruntulenme)
goruntulenme++
Sorular.findByIdAndUpdate(id,{goruntulenme : goruntulenme}).then((gresult)=>{
res.render(__dirname+"/src/signed/soru.ejs", {title : result.title, kullanici : kresult, soru : result, yorum : yresult})  
})
})
} else {
res.render(__dirname+"/src/pages/soru.ejs", {title : result.title, soru : result, yorum : yresult})  
}
})
})
}).catch((err)=>{console.log(err)})
...
Kullanicilar.find({kullanici_adi : kullanici_adi, sifre : sifre}).then((result)=>{
Kullanicilar.findOne({kullanici_adi : kullanici_adi, sifre : sifre},(req,res,user)=>{
if(user){
...
res.render(__dirname+'/src/admin/edit-soru.ejs', {title : "Soru Düzenleme", soru : result, kullanici: kresult})
})
})
})
